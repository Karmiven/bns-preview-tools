...
namespace Xylia.Preview.Output
{
	public sealed class ItemCloset_Type : OutBase
	{
		...
		protected override void CreateData()
		{
			...
			this.ExcelInfo.SetColumn("种族", 10);
			this.ExcelInfo.SetColumn("性别", 10);
			this.ExcelInfo.SetColumn("定制类型", 20);
		...
		{
			var sheet = this.ExcelInfo.CreateSheet(SheetName);
			this.ExcelInfo.SetColumn("物品编号", 15);
			this.ExcelInfo.SetColumn("物品别名", 65);
			this.ExcelInfo.SetColumn("物品名称", 30);
			this.ExcelInfo.SetColumn("装备类型", 25);
			this.ExcelInfo.SetColumn("衣柜编号", 15);
			this.ExcelInfo.SetColumn("衣柜目录", 15);
		...
		{
			var row = this.ExcelInfo.CreateRow(sheet: Sheet);
